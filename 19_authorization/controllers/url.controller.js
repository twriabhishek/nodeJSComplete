...
const generateShortUrl = async (req, res) => {
  ...
  if (existingUrl) {
    return res.status(409).json({ error: "Duplicate URL for this user" });
  }

  // If URL does not exist for the current user, proceed with creation
...
const getRedirectUrl = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ RedirectUrl: urlDoc.redirectUrl});
...
const getAllShortUrlId = async(req, res) =>{
  const allShortUrls = await URL.find({createdBy: req.user._id}, '_id shortId redirectUrl');
